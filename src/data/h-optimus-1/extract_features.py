"""
H-Optimus-1 Feature Extraction for Thyroid TERT Dataset (DDP)
...
# DataLoader worker가 공유 텐서를 파일 디스크립터로 주고받는 기본 방식은
# 타일 수가 많은 슬라이드에서 fd 한도를 넘겨 "Too many open files"로 죽는다.
# 파일시스템 기반 공유로 바꾸면 fd를 소모하지 않는다.
